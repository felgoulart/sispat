...
<title><h:outputText value="#{cadAssetVhrdvBean.trans_tt}"/></title>
</head>
<body onkeydown="rk();"  onload="window.focus();loading();ufmvini();tran='assetvhrdvcad';cadbeanx='cadAssetVhrdvBean'; loadPage(<%= request.getAttribute("pagina") %>); AlternarAbas(parent.cadbeanx, <%=(Object)session.getAttribute(("cadAssetVhrdvBeanx"))%>);AlternarSubAbas();">
...
<div id="loading" class="loading" align="center"><h:outputText id="label_wait" value="#{iimsg.msg_wait}"/></div>
	<div class="container expand-max bg-transparent mt-3">
	  <div class="row">
		...
		<div class="col-sm-10">
		<h2 class="title" style="font-size: 35px;"><h:outputText styleClass="h2 mt-3"  value="#{cadAssetVhrdvBean.trans_tt}"/></h2>
		...
		<p class="subtitle" style="text-align:center;color:#505050">
			...
			<h:outputText id="asset_id11" value="#{cadAssetVhrdvBean.asset_id}"  />
			<h:outputText value="#{iimsg.AssetBldng_asset_nu_tt}"/>
			<h:outputText id="asset_id_dsc11" value="#{cadAssetVhrdvBean.asset_nu}" />
		</p>
		<p style="text-align:center;">
		<h:inputText id="mesg_varastif1" rendered="#{cadAssetVhrdvBean.canUpdate}" value="#{cadAssetVhrdvBean.mesg_var}"  styleClass="txrr pr5 tb450r" tabindex="-1" readonly="true"/>
		<h:commandButton id="aMessageastif1" image="../imagens/btMessage_off.gif" styleClass="d-none" title="#{iimsg.botao_message}" rendered="#{cadAssetVhrdvBean.canUpdate}" action="#{cadAssetVhrdvBean.doValidate}" onmousedown="msgw('aMessageastif1');"/>
		</p>
		</div>
		<div class="col-sm-1">
		<h:commandButton id="aBackToSearch1" immediate="true"  styleClass="btn btn-outline-dark" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetbldngpsq'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToPrevious_tt}" value="Voltar" action="#{cadAssetVhrdvBean.doSelecionar}"/>
		</div>
	</div>
	</div>
...
<h:commandButton id="aBackToSearch1Menu" immediate="true"  styleClass="d-none" image="../imagens/btReturnxMenu.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetvhrdvcad'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToMenu_tt}" action="#{cadAssetVhrdvBean.doReturnMenu}"/>
<h:commandButton id="aUpdate" styleClass="d-none" rendered="#{cadAssetVhrdvBean.canUpdate}" image="../imagens/btSave.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetvhrdvcad'+'&ufmv='+stx;msgwr();upflag=0;AlternarTabsvf();" title="#{iimsg.botao_update}" action="#{cadAssetVhrdvBean.doUpdate}"/>
<h:commandButton id="aCancelar" styleClass="d-none" rendered="#{cadAssetVhrdvBean.canUpdate}" image="../imagens/btNew.gif"  disabled="" onmousedown="msgwr()" title="#{iimsg.botao_novo}" action="#{cadAssetVhrdvBean.doCancelar}"/>
<h:commandButton id="aExcluir" styleClass="d-none" title="#{iimsg.botao_excluir}" rendered="#{cadAssetVhrdvBean.canExcluir}" image="../imagens/btDelete.gif"  disabled="" onmousedown="msgwex()" action="#{cadAssetVhrdvBean.doExcluir}"/>
<h:commandButton id="aRefresh" styleClass="d-none"  image="../imagens/btRefresh.gif" title="#{iimsg.botao_refresh}"  action="#{cadAssetVhrdvBean.doObter}"  onmousedown="msgwr();document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetvhrdvcad'+'&ufmv='+stx"/>
...
<h:commandButton id="aHelpAssetComRollsRfm" styleClass="d-none" tabindex="-1" immediate="true" rendered="#{cadAssetVhrdvBean.hasHelpComRfm}" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes', 'AssetComRollsRfm');" image="../imagens/btHelpCom.gif"/>
<h:commandButton id="aHelpAssetRollsRfm" styleClass="d-none" tabindex="-1" immediate="true" rendered="#{cadAssetVhrdvBean.hasHelpSpcRfm}" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes', 'AssetRollsRfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:inputHidden id="tabsvf" value="#{cadAssetVhrdvBean.tabsvf}" />
<h:inputHidden id="xhelp_rc" value="#{cadAssetVhrdvBean.xhelp_rc}" />
<h:inputHidden id="tabsvf_in" value="#{cadAssetVhrdvBean.tabsvf_in}" />
<h:inputHidden id="tabsvf_tr" value="#{cadAssetVhrdvBean.tabsvf_tr}" />
<h:inputHidden id="xbutn_id" value="#{cadAssetVhrdvBean.xbutn_id}" />
<h:inputHidden id="xbutx_id" value="" />
<h:inputHidden id="xuserx_id_login" value="#{cadAssetVhrdvBean.xuserx_id_login}" />
<h:inputHidden id="xmesg_rc" value="#{cadAssetVhrdvBean.xmesg_rc}" />
<h:inputHidden id="xmesg_st" value="#{cadAssetVhrdvBean.xmesg_st}" />
...
<h:outputText value="#{iimsg.AssetVhrdv_D00_INFGR_UV_st}"/>
</h:outputLink>
</td>
...
<h:outputText value="#{iimsg.AssetVhrdv_D00_CTB_UV_st}"/>
</h:outputLink>
</td>
...
<h:outputText value="#{iimsg.AssetVhrdv_D00_STATUS_UV_st}"/>
</h:outputLink>
</td>
...
<h:commandLink id="xsel_vali" action="#{cadAssetVhrdvBean.doValidate}" immediate="true"/>
