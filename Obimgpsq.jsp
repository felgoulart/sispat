...
<%session.setAttribute("cadbeanx","cadObimgBean");%>
...
<link rel="stylesheet" href="../Obimgpsq.css" type="text/css">
...
<title>
<h:outputText value="#{cadObimgBean.trans_tt}"/>
</title>
</head>
<body onload="window.focus();loadPage();loading();helpcd='ObimgSfm';;tran='obimgpsq'; vl='';cadbeanx='cadObimgBean';<%session.setAttribute("tipo","obimgpsq");%>">
<h:form id="cadForm">
...
<h:commandButton id="aBackToSearch" styleClass="btp" image="../imagens/btVoltar.gif" onmousedown="document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+''+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToPrevious_tt}" action="#{cadObimgBean.doReturnMenu}"/>
<h:commandButton id="aPrev" styleClass="btp"  image="../imagens/btPgAnterior.gif" title="#{iimsg.botao_anterior}" rendered="#{cadObimgBean.hasPrevPage}" action="#{cadObimgBean.doPaginaAnterior}"/>
<h:commandButton id="aNext" styleClass="btp"  image="../imagens/btPgProxima.gif" title="#{iimsg.botao_proxima}" rendered="#{cadObimgBean.hasNextPage}" action="#{cadObimgBean.doProximaPagina}"/>
<h:outputText id="label_fill" style="display:none" value="#{iimsg.label_fill}" />
<h:outputText id="label_notfound" style="display:none" value="#{iimsg.label_notfound}" />
<h:outputText id="sys_openning_form" style="display:none" value="#{iimsg.sys_openning_form}" />
<h:commandButton id="aFiltrar" styleClass="btp"  image="../imagens/btFilter.gif" title="#{iimsg.botao_filtrar}" action="#{cadObimgBean.doSelecionar}"/>
<h:commandButton id="aLimpar" styleClass="btp" rendered="#{cadObimgBean.canPesquisar}" image="../imagens/btCancel.gif" title="#{iimsg.botao_limpar}" action="#{cadObimgBean.doLimparPesq}"/>
<h:commandButton id="aNew" styleClass="btp" rendered="#{cadObimgBean.canInserir}" image="../imagens/btNew.gif" onmousedown="fNovorwpp('obimgcad', tran, this.id, 'yes', fPgAssist());" title="#{iimsg.botao_novo}" action="#{cadObimgBean.doObterParentForCancel}"/>
...
<h:commandButton id="aHelpObimgSfm" tabindex="-1" rendered="#{cadObimgBean.hasHelpSpcSfm}" immediate="true" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes','ObimgSfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:outputLabel  styleClass="tip txr pl0"  id="lbl_obimg_id" for="obimg_id"  title="#{iimsg.Obimg_obimg_id_tt}"  value="#{iimsg.Obimg_obimg_id_tt}"/>  </td> <%-- <top> --%>
<td      width="400" >
<h:inputText id="obimg_id" tabindex="-1" readonly="true" value="#{cadObimgBean.obimg_id}" size="20" maxlength="20" onkeydown="keypress=1;"  styleClass="tbxr"/>
</td> <%-- <close td> pKey --%>
</td> <%-- <close td> Button --%>
</tr> <%-- <tr close> pKey--%>
...
<h:inputText id="mesg_var" value="#{cadObimgBean.mesg_var}"  styleClass="txrr pr5 tb600r" tabindex="-1" readonly="true"/>
</td> <%-- <close td> msg --%>
<td> &nbsp&nbsp </td>
<td      width="50" > <%-- open w/ inputText 222--%>
<%-- outputLabel msg --%>
...
<%-- outputText --%>
<td    width="100" > <h:inputText id="codfqt" onfocus="this.blur()" value="#{cadObimgBean.codfqt}" size="5" maxlength="5"  styleClass="txl pl5 tbxr"/>
</td> <%-- <close td> msg --%>
</tr> <%-- <tr close> msg--%>
...
<h:inputText id="fil_obimg_tp_cfasttp_xfil" title="#{iimsg.hint_dblclick}" ondblclick="pw('C'+'ASTTP', tran, this.id);" onblur="cfb(this.id, 'pfcfb9', 'pfcf9');" onfocus="this.className='pfcf9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObimgBean.fil_obimg_tp_cfasttp}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obimg_ds_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObimgBean.fil_obimg_ds}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obimg_vc_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObimgBean.fil_obimg_vc}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- recdWdColumnGen -->
...
<!-- Dados -->
<h:dataTable
value="#{cadObimgBean.pesqData}"
var="Obimg"
binding="#{cadObimgBean.pesqTable}"
rowClasses="tre9, tro9"
headerClass="tbh9"
footerClass="tbf9"
id="dtDados"
rows="#{cadObimgBean.pageLength}"
first="#{cadObimgBean.currentLine}">
<!-- propColumnGen -->
<h:column id="obimg_nm_file">
...
<h:commandLink id="flag_imgObimg" onmousedown="wtran='obimgcad';el('hiddenform').target=tran;aa=this.id.slice(0,this.id.search('flag_img'));valor1=el(aa+'sel_obimg_id').value;valor2=el(aa+'sel_obimg_sq').value;el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;showpopup(tran,'xrec_obob',this.id,wtran,'yes');">
...
<h:inputHidden id="sel_obimg_id" value="#{Obimg.obimg_id}"/>
<h:inputHidden id="sel_obimg_sq" value="#{Obimg.obimg_sq}"/>
<!-- BBB--> <h:commandLink id="flag_imgrecd_wd" action="#{cadObimgBean.doObternullSearchBack}" onmousedown="wtran='obimgcad';aa=this.id.slice(0,this.id.search('flag_img'));valor1=rpel(aa+'sel_obimg_id');valor2=rpel(aa+'sel_obimg_sq');selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;">
...
<h:commandLink id="xrec_obnv" action="#{cadObimgBean.doObterParentForCancel}" immediate="true"/>
