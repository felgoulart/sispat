...
<link rel="stylesheet" href="../Obdocpjckloppsq.css" type="text/css">
...
<body onload="window.focus();loadPage();loading();helpcd='ObdocpjcklopSfm';;tran='obdocpjckloppsq'; vl='';cadbeanx='cadObdocpjcklopBean';<%session.setAttribute("tipo","obdocpjckloppsq");%>">
<h:form id="cadForm">
...
<h:commandButton id="aBackToSearchMenu" styleClass="btp" image="../imagens/btReturnxMenu.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'obdocpjcklopcad'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToMenu_tt}" action="#{cadObdocpjcklopBean.doReturnMenu}"/>
<h:commandButton id="aPrev" styleClass="btp"  image="../imagens/btPgAnterior.gif" title="#{iimsg.botao_anterior}" rendered="#{cadObdocpjcklopBean.hasPrevPage}" action="#{cadObdocpjcklopBean.doPaginaAnterior}"/>
<h:commandButton id="aNext" styleClass="btp"  image="../imagens/btPgProxima.gif" title="#{iimsg.botao_proxima}" rendered="#{cadObdocpjcklopBean.hasNextPage}" action="#{cadObdocpjcklopBean.doProximaPagina}"/>
<h:outputText id="label_fill" style="display:none" value="#{iimsg.label_fill}" />
<h:outputText id="label_notfound" style="display:none" value="#{iimsg.label_notfound}" />
<h:outputText id="sys_openning_form" style="display:none" value="#{iimsg.sys_openning_form}" />
<h:commandButton id="aFiltrar" styleClass="btp"  image="../imagens/btFilter.gif" title="#{iimsg.botao_filtrar}" action="#{cadObdocpjcklopBean.doSelecionar}"/>
<h:commandButton id="aLimpar" styleClass="btp" rendered="#{cadObdocpjcklopBean.canPesquisar}" image="../imagens/btCancel.gif" title="#{iimsg.botao_limpar}" action="#{cadObdocpjcklopBean.doLimparPesq}"/>
<h:commandButton id="aNew" styleClass="btp" rendered="#{cadObdocpjcklopBean.canInserir}" image="../imagens/btNew.gif" onmousedown="fNovorwpp('obdocpjcklopcad', tran, this.id, 'yes', fPgAssist());" title="#{iimsg.botao_novo}" action="#{cadObdocpjcklopBean.doNovoUpload}"/>
...
<h:commandButton id="aHelpObdocpjcklopSfm" tabindex="-1" rendered="#{cadObdocpjcklopBean.hasHelpSpcSfm}" immediate="true" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes','ObdocpjcklopSfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:outputLabel  styleClass="tip txr pl0"  id="lbl_obdocpj_id" for="obdocpj_id"  title="#{iimsg.Obdocpjcklop_obdocpj_id_tt}"  value="#{iimsg.Obdocpjcklop_obdocpj_id_tt}"/>  </td> <%-- <top> --%>
<td      width="400" >
<h:inputText id="obdocpj_id" tabindex="-1" readonly="true" value="#{cadObdocpjcklopBean.obdocpj_id}" size="20" maxlength="20" onkeydown="keypress=1;"  styleClass="tbxr"/>
<h:commandLink id="obdocpj_id_desc" value="#{cadObdocpjcklopBean.obdocpj_id_desc}" styleClass="dsp"  onmousedown="pwfk('assetcad', tran, this.id, 'no')"/>
</td> <%-- <close td> pKey --%>
<td   ><h:outputLabel  styleClass="tip txl pl0"  id="lbl_p_seltype" for="p_seltype"  value="#{iimsg.Obdocpjcklop_p_seltype_tt}"/>
</td> <%-- <close td> selectBoolean --%>
<td   width="200"   >
<h:inputHidden id="p_seltype_bol" value="#{cadObdocpjcklopBean.p_seltype}"/>
<h:selectBooleanCheckbox id="p_seltype" value="#{cadObdocpjcklopBean.p_seltype_bol}" onclick="chkBoolean(this, 'cadForm:p_seltype_bol');"/>
</td> <%-- <close td> selectBoolean --%>
<td   > <h:commandButton id="PackFilesFromSList" title="#{iimsg.Obdocpjcklop_PackFilesFromSList_tt}" value="#{iimsg.Obdocpjcklop_PackFilesFromSList_ta}" styleClass="btnBox160"  action="#{cadObdocpjcklopBean.PackFilesFromSList}" onmousedown="ckbSelListx__a('obdocpj_nm_file')"/>
...
<h:inputText id="mesg_var" value="#{cadObdocpjcklopBean.mesg_var}"  styleClass="txrr pr5 tb600r" tabindex="-1" readonly="true"/>
</td> <%-- <close td> msg --%>
<td> &nbsp&nbsp </td>
<td      width="50" > <%-- open w/ inputText 222--%>
<%-- outputLabel msg --%>
...
<%-- outputText --%>
<td    width="100" > <h:inputText id="codfqt" onfocus="this.blur()" value="#{cadObdocpjcklopBean.codfqt}" size="5" maxlength="5"  styleClass="txl pl5 tbxr"/>
</td> <%-- <close td> msg --%>
</tr> <%-- <tr close> msg--%>
...
<h:inputText id="fil_obdocpj_cl_type_xfil" title="#{iimsg.hint_dblclick}" ondblclick="pw('C'+'CKLOP', tran, this.id);" onblur="cfb(this.id, 'pfcfb9', 'pfcf9');" onfocus="this.className='pfcf9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObdocpjcklopBean.fil_obdocpj_cl_type}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- codfColumnGen -->
...
<h:inputText id="fil_obdocpj_st_xfil" title="#{iimsg.hint_dblclick}" ondblclick="pw('C'+'STCKL', tran, this.id);" onblur="cfb(this.id, 'pfcfb9', 'pfcf9');" onfocus="this.className='pfcf9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObdocpjcklopBean.fil_obdocpj_st}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obdocpj_ds_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObdocpjcklopBean.fil_obdocpj_ds}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obdocpj_vc_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObdocpjcklopBean.fil_obdocpj_vc}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- recdWdColumnGen -->
...
<!-- Dados -->
<h:dataTable
value="#{cadObdocpjcklopBean.pesqData}"
var="Obdocpj"
binding="#{cadObdocpjcklopBean.pesqTable}"
rowClasses="tre9, tro9"
headerClass="tbh9"
footerClass="tbf9"
id="dtDados"
rows="#{cadObdocpjcklopBean.pageLength}"
first="#{cadObdocpjcklopBean.currentLine}">
<!-- codfColumnGen -->
...
<h:commandLink id="flag_imgObdocpjcklop" onmousedown="wtran='obdocpjcklopcad';el('hiddenform').target=tran;aa=this.id.slice(0,this.id.search('flag_img'));valor1=el(aa+'sel_obdocpj_id').value;valor2=el(aa+'sel_obdocpj_sq').value;el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;showpopup(tran,'xrec_obob',this.id,wtran,'yes');">
...
<!-- CRC-30-->  <h:commandButton id="flag_imgrecd_wd" image="../imagens/btRecd.gif" action="#{cadObdocpjcklopBean.doObternullSearchBack}" onmousedown="wtran='obdocpjcklopcad';aa=this.id.slice(0,this.id.search('flag_img'));valor1=rpel(aa+'sel_obdocpj_id');valor2=rpel(aa+'sel_obdocpj_sq');selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;"/>
</h:panelGroup>
</h:column>
<!-- propSearchChildTableGen -->
</h:dataTable>
...
<h:commandLink id="xrec_obnv" action="#{cadObdocpjcklopBean.doObterParentForCancel}" immediate="true"/>
