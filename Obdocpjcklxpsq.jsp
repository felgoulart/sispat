...
<link rel="stylesheet" href="../Obdocpjcklxpsq.css" type="text/css">
...
<body onload="window.focus();loadPage();loading();helpcd='ObdocpjcklxSfm';;tran='obdocpjcklxpsq'; vl='';cadbeanx='cadObdocpjcklxBean';<%session.setAttribute("tipo","obdocpjcklxpsq");%>">
<h:form id="cadForm">
...
<h:commandButton id="aBackToSearchMenu" styleClass="btp" image="../imagens/btReturnxMenu.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'obdocpjcklxcad'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToMenu_tt}" action="#{cadObdocpjcklxBean.doReturnMenu}"/>
<h:commandButton id="aPrev" styleClass="btp"  image="../imagens/btPgAnterior.gif" title="#{iimsg.botao_anterior}" rendered="#{cadObdocpjcklxBean.hasPrevPage}" action="#{cadObdocpjcklxBean.doPaginaAnterior}"/>
<h:commandButton id="aNext" styleClass="btp"  image="../imagens/btPgProxima.gif" title="#{iimsg.botao_proxima}" rendered="#{cadObdocpjcklxBean.hasNextPage}" action="#{cadObdocpjcklxBean.doProximaPagina}"/>
<h:outputText id="label_fill" style="display:none" value="#{iimsg.label_fill}" />
<h:outputText id="label_notfound" style="display:none" value="#{iimsg.label_notfound}" />
<h:outputText id="sys_openning_form" style="display:none" value="#{iimsg.sys_openning_form}" />
<h:commandButton id="aFiltrar" styleClass="btp"  image="../imagens/btFilter.gif" title="#{iimsg.botao_filtrar}" action="#{cadObdocpjcklxBean.doSelecionar}"/>
<h:commandButton id="aLimpar" styleClass="btp" rendered="#{cadObdocpjcklxBean.canPesquisar}" image="../imagens/btCancel.gif" title="#{iimsg.botao_limpar}" action="#{cadObdocpjcklxBean.doLimparPesq}"/>
...
<h:commandButton id="aHelpObdocpjcklxSfm" tabindex="-1" rendered="#{cadObdocpjcklxBean.hasHelpSpcSfm}" immediate="true" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes','ObdocpjcklxSfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:inputText id="mesg_var" value="#{cadObdocpjcklxBean.mesg_var}"  styleClass="txrr pr5 tb600r" tabindex="-1" readonly="true"/>
</td> <%-- <close td> msg --%>
<td> &nbsp&nbsp </td>
<td      width="50" > <%-- open w/ inputText 222--%>
<%-- outputLabel msg --%>
...
<%-- outputText --%>
<td    width="100" > <h:inputText id="codfqt" onfocus="this.blur()" value="#{cadObdocpjcklxBean.codfqt}" size="5" maxlength="5"  styleClass="txl pl5 tbxr"/>
</td> <%-- <close td> msg --%>
</tr> <%-- <tr close> msg--%>
...
<h:outputText id="hd_obdocpj_id_tt" title="#{iimsg.Obdocpjckl_obdocpj_id_tt}" value="#{iimsg.Obdocpjckl_obdocpj_id_ta}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- codfColumnGen -->
...
<!--   // 444-->
<h:inputText id="obdocpj_id_xfil" title="#{iimsg.hint_dblclick}" ondblclick="this.value=''" onblur="cfb(this.id, 'pfdb9', 'pfd9');" onfocus="this.className='pfd9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObdocpjcklxBean.fil_obdocpj_id}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- codfColumnGen -->
...
<h:inputText id="fil_obdocpj_cl_type_xfil" title="#{iimsg.hint_dblclick}" ondblclick="pw('C'+'TPCKL', tran, this.id);" onblur="cfb(this.id, 'pfcfb9', 'pfcf9');" onfocus="this.className='pfcf9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObdocpjcklxBean.fil_obdocpj_cl_type}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- codfColumnGen -->
...
<h:inputText id="fil_obdocpj_st_xfil" title="#{iimsg.hint_dblclick}" ondblclick="pw('C'+'STCKL', tran, this.id);" onblur="cfb(this.id, 'pfcfb9', 'pfcf9');" onfocus="this.className='pfcf9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObdocpjcklxBean.fil_obdocpj_st}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obdocpj_ds_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObdocpjcklxBean.fil_obdocpj_ds}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obdocpj_vc_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObdocpjcklxBean.fil_obdocpj_vc}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- recdWdColumnGen -->
...
<!-- Dados -->
<h:dataTable
value="#{cadObdocpjcklxBean.pesqData}"
var="Obdocpj"
binding="#{cadObdocpjcklxBean.pesqTable}"
rowClasses="tre9, tro9"
headerClass="tbh9"
footerClass="tbf9"
id="dtDados"
rows="#{cadObdocpjcklxBean.pageLength}"
first="#{cadObdocpjcklxBean.currentLine}">
<!-- fkeyColumnGen -->
...
<h:commandLink id="flag_imgObdocpjckl" onmousedown="wtran='obdocpjcklcad';el('hiddenform').target=tran;aa=this.id.slice(0,this.id.search('flag_img'));valor1=el(aa+'sel_obdocpj_id').innerHTML;valor2=el(aa+'sel_obdocpj_sq').value;el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;showpopup(tran,'xrec_obob',this.id,wtran,'yes');">
...
<!-- CRX-30-->  <h:commandButton id="flag_imgrecd_wd" image="../imagens/btRecd.gif" action="#{cadObdocpjcklxBean.doObternullSearchBack}" onmousedown="wtran='obdocpjcklxcad';aa=this.id.slice(0,this.id.search('flag_img'));valor1=rpel(aa+'sel_obdocpj_id');valor2=rpel(aa+'sel_obdocpj_sq');selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;"/>
</h:panelGroup>
</h:column>
<!-- recdWdColumnGen -->
<h:column id="recd_wdpk">
...
<f:param name="tipoant" value="obdocpjcklxpsq"/>
</h:commandLink>
...
<h:commandLink id="xrec_obnv" action="#{cadObdocpjcklxBean.doObterParentForCancel}" immediate="true"/>
