...
<link rel="stylesheet" href="../Obaedesgpsq.css" type="text/css">
...
function fPgAssist() {
page_assist = <%= session.getAttribute("obaedesgpsq") %>;
return page_assist;
}
function loadPage() {
hovDtTable('dtDados');
...
<body onload="window.focus();loadPage();loading();helpcd='ObaedesgSfm';;tran='obaedesgpsq'; vl='';cadbeanx='cadObaedesgBean';<%session.setAttribute("tipo","obaedesgpsq");%>">
<h:form id="cadForm">
...
<h:commandButton id="aBackToSearchMenu" styleClass="btp" image="../imagens/btReturnxMenu.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'obaedesgcad'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToMenu_tt}" action="#{cadObaedesgBean.doReturnMenu}"/>
<h:commandButton id="aPrev" styleClass="btp"  image="../imagens/btPgAnterior.gif" title="#{iimsg.botao_anterior}" rendered="#{cadObaedesgBean.hasPrevPage}" action="#{cadObaedesgBean.doPaginaAnterior}"/>
<h:commandButton id="aNext" styleClass="btp"  image="../imagens/btPgProxima.gif" title="#{iimsg.botao_proxima}" rendered="#{cadObaedesgBean.hasNextPage}" action="#{cadObaedesgBean.doProximaPagina}"/>
<h:outputText id="label_fill" style="display:none" value="#{iimsg.label_fill}" />
<h:outputText id="label_notfound" style="display:none" value="#{iimsg.label_notfound}" />
<h:outputText id="sys_openning_form" style="display:none" value="#{iimsg.sys_openning_form}" />
<h:commandButton id="aFiltrar" styleClass="btp"  image="../imagens/btFilter.gif" title="#{iimsg.botao_filtrar}" action="#{cadObaedesgBean.doSelecionar}"/>
<h:commandButton id="aLimpar" styleClass="btp" rendered="#{cadObaedesgBean.canPesquisar}" image="../imagens/btCancel.gif" title="#{iimsg.botao_limpar}" action="#{cadObaedesgBean.doLimparPesq}"/>
<h:commandButton id="aNew" styleClass="btp" rendered="#{cadObaedesgBean.canInserir}" image="../imagens/btNew.gif" onmousedown="fNovorwpp('obaedesgcad', tran, this.id, 'yes', fPgAssist());" title="#{iimsg.botao_novo}" action="#{cadObaedesgBean.doObterParentForCancel}"/>
...
<h:commandButton id="aHelpObaedesgSfm" tabindex="-1" rendered="#{cadObaedesgBean.hasHelpSpcSfm}" immediate="true" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes','ObaedesgSfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:outputLabel  styleClass="tip txr pl0"  id="lbl_obaed_id" for="obaed_id"  title="#{iimsg.Obaedesg_obaed_id_tt}"  value="#{iimsg.Obaedesg_obaed_id_tt}"/>  </td> <%-- <top> --%>
<td      width="400" >
<h:inputText id="obaed_id" tabindex="-1" readonly="true" value="#{cadObaedesgBean.obaed_id}" size="20" maxlength="20" onkeydown="keypress=1;"  styleClass="tbxr"/>
<h:commandLink id="obaed_id_desc" value="#{cadObaedesgBean.obaed_id_desc}" styleClass="dsp"  onmousedown="pwfk('assetcad', tran, this.id, 'no')"/>
</td> <%-- <close td> pKey --%>
</td> <%-- <close td> Button --%>
</tr> <%-- <tr close> pKey--%>
...
<h:inputText id="mesg_var" value="#{cadObaedesgBean.mesg_var}"  styleClass="txrr pr5 tb600r" tabindex="-1" readonly="true"/>
</td> <%-- <close td> msg --%>
<td> &nbsp&nbsp </td>
<td      width="50" > <%-- open w/ inputText 222--%>
<%-- outputLabel msg --%>
...
<%-- outputText --%>
<td    width="100" > <h:inputText id="codfqt" onfocus="this.blur()" value="#{cadObaedesgBean.codfqt}" size="5" maxlength="5"  styleClass="txl pl5 tbxr"/>
</td> <%-- <close td> msg --%>
</tr> <%-- <tr close> msg--%>
...
<h:outputText id="hd_obaed_vl_tt" title="#{iimsg.Obaedesg_obaed_vl_tt}" value="#{iimsg.Obaedesg_obaed_vl_ta}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:outputText id="hd_tabs_vc_tt_xide" title="#{iimsg.Obaedesg_tabs_vc_tt}" value="#{iimsg.Obaedesg_tabs_vc_ta}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- recdWdColumnGen -->
...
<h:inputText id="tabs_vc_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObaedesgBean.fil_tabs_vc}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- recdWdColumnGen -->
...
<!-- Dados -->
<h:dataTable
value="#{cadObaedesgBean.pesqData}"
var="Obaed"
binding="#{cadObaedesgBean.pesqTable}"
rowClasses="tre9, tro9"
headerClass="tbh9"
footerClass="tbf9"
id="dtDados"
rows="#{cadObaedesgBean.pageLength}"
first="#{cadObaedesgBean.currentLine}">
<!-- codfColumnGen -->
...
<h:commandLink id="flag_imgObaedesg" onmousedown="wtran='obaedesgcad';el('hiddenform').target=tran;aa=this.id.slice(0,this.id.search('flag_img'));valor1=el(aa+'sel_obaed_id').value;valor2=el(aa+'sel_obaed_sq').value;el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;showpopup(tran,'xrec_obob',this.id,wtran,'yes');">
...
<!-- CRC-30-->  <h:commandButton id="flag_imgrecd_wd" image="../imagens/btRecd.gif" action="#{cadObaedesgBean.doObternullSearchBack}" onmousedown="wtran='obaedesgcad';aa=this.id.slice(0,this.id.search('flag_img'));valor1=rpel(aa+'sel_obaed_id');valor2=rpel(aa+'sel_obaed_sq');selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;"/>
</h:panelGroup>
</h:column>
<!-- propSearchChildTableGen -->
</h:dataTable>
...
<!-- Conteudo -->
<iframe id="Obaedesg" width="0" height="0" frameborder="0"> </iframe>
...
<h:commandLink id="xrec_obnv" action="#{cadObaedesgBean.doObterParentForCancel}" immediate="true"/>
