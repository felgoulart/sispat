...
<link rel="stylesheet" href="../Pjhsocontpsq.css" type="text/css">
...
<body onload="window.focus();loadPage();loading();helpcd='PjhsocontSfm';;tran='pjhsocontpsq'; vl='';cadbeanx='cadPjhsocontBean';<%session.setAttribute("tipo","pjhsocontpsq");%>">
<h:form id="cadForm">
...
<h:commandButton id="aBackToSearchMenu" styleClass="btp" image="../imagens/btReturnxMenu.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'pjhsocontcad'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToMenu_tt}" action="#{cadPjhsocontBean.doReturnMenu}"/>
<h:commandButton id="aPrev" styleClass="btp"  image="../imagens/btPgAnterior.gif" title="#{iimsg.botao_anterior}" rendered="#{cadPjhsocontBean.hasPrevPage}" action="#{cadPjhsocontBean.doPaginaAnterior}"/>
<h:commandButton id="aNext" styleClass="btp"  image="../imagens/btPgProxima.gif" title="#{iimsg.botao_proxima}" rendered="#{cadPjhsocontBean.hasNextPage}" action="#{cadPjhsocontBean.doProximaPagina}"/>
<h:outputText id="label_fill" style="display:none" value="#{iimsg.label_fill}" />
<h:outputText id="label_notfound" style="display:none" value="#{iimsg.label_notfound}" />
<h:outputText id="sys_openning_form" style="display:none" value="#{iimsg.sys_openning_form}" />
<h:commandButton id="aFiltrar" styleClass="btp"  image="../imagens/btFilter.gif" title="#{iimsg.botao_filtrar}" action="#{cadPjhsocontBean.doSelecionar}"/>
<h:commandButton id="aLimpar" styleClass="btp" rendered="#{cadPjhsocontBean.canPesquisar}" image="../imagens/btCancel.gif" title="#{iimsg.botao_limpar}" action="#{cadPjhsocontBean.doLimparPesq}"/>
<h:commandButton id="aNew" styleClass="btp" rendered="#{cadPjhsocontBean.canInserir}" image="../imagens/btNew.gif" onmousedown="fNovorwpp('pjhsocontcad', tran, this.id, 'yes', fPgAssist());" title="#{iimsg.botao_novo}" action="#{cadPjhsocontBean.doNovo}"/>
...
<h:commandButton id="aHelpPjhsocontSfm" tabindex="-1" rendered="#{cadPjhsocontBean.hasHelpSpcSfm}" immediate="true" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes','PjhsocontSfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:inputText id="mesg_var" value="#{cadPjhsocontBean.mesg_var}"  styleClass="txrr pr5 tb600r" tabindex="-1" readonly="true"/>
</td> <%-- <close td> msg --%>
<td> &nbsp&nbsp </td>
<td      width="50" > <%-- open w/ inputText 222--%>
<%-- outputLabel msg --%>
...
<%-- outputText --%>
<td    width="100" > <h:inputText id="codfqt" onfocus="this.blur()" value="#{cadPjhsocontBean.codfqt}" size="5" maxlength="5"  styleClass="txl pl5 tbxr"/>
</td> <%-- <close td> msg --%>
</tr> <%-- <tr close> msg--%>
...
<h:inputText id="tabs_vc_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadPjhsocontBean.fil_tabs_vc}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- recdWdColumnGen -->
...
<!-- Dados -->
<h:dataTable
value="#{cadPjhsocontBean.pesqData}"
var="Pjhso"
binding="#{cadPjhsocontBean.pesqTable}"
rowClasses="tre9, tro9"
headerClass="tbh9"
footerClass="tbf9"
id="dtDados"
rows="#{cadPjhsocontBean.pageLength}"
first="#{cadPjhsocontBean.currentLine}">
<!-- propColumnGen -->
<h:column id="tabs_vc">
...
<!-- CRC-30-->  <h:commandButton id="flag_imgrecd_wd" image="../imagens/btRecd.gif" action="#{cadPjhsocontBean.doObternullSearchBack}" onmousedown="wtran='pjhsocontcad';aa=this.id.slice(0,this.id.search('flag_img'));valor1=rpel(aa+'sel_pjhso_id_pjaus');valor2=rpel(aa+'sel_pjhso_sq');selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;"/>
</h:panelGroup>
</h:column>
<!-- propSearchChildTableGen -->
</h:dataTable>
...
<h:commandLink id="xrec_obnv" action="#{cadPjhsocontBean.doNovo}" immediate="true"/>
