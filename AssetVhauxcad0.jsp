...
<title><h:outputText value="#{cadAssetVhauxBean.trans_tt}"/></title>
</head>
<body onkeydown="rk();"  onload="window.focus();loading();ufmvini();tran='assetvhauxcad';cadbeanx='cadAssetVhauxBean'; loadPage(<%= request.getAttribute("pagina") %>); AlternarAbas(parent.cadbeanx, <%=(Object)session.getAttribute(("cadAssetVhauxBeanx"))%>);AlternarSubAbas();">
...
<div id="loading" class="loading" align="center"><h:outputText id="label_wait" value="#{iimsg.msg_wait}"/></div>
	<div class="container expand-max bg-transparent mt-3">
	  <div class="row">
		...
		<div class="col-sm-10">
		<h2 class="title" style="font-size: 35px;"><h:outputText styleClass="h2 mt-3"  value="#{cadAssetVhauxBean.trans_tt}"/></h2>
		...
		<p class="subtitle" style="text-align:center;color:#505050">
			...
			<h:outputText id="asset_id11" value="#{cadAssetVhauxBean.asset_id}"  />
			<h:outputText value="#{iimsg.AssetBldng_asset_nu_tt}"/>
			<h:outputText id="asset_id_dsc11" value="#{cadAssetVhauxBean.asset_nu}" />
		</p>
		<p style="text-align:center;">
		<h:inputText id="mesg_varastif1" rendered="#{cadAssetVhauxBean.canUpdate}" value="#{cadAssetVhauxBean.mesg_var}"  styleClass="txrr pr5 tb450r" tabindex="-1" readonly="true"/>
		<h:commandButton id="aMessageastif1" image="../imagens/btMessage_off.gif" styleClass="d-none" title="#{iimsg.botao_message}" rendered="#{cadAssetVhauxBean.canUpdate}" action="#{cadAssetVhauxBean.doValidate}" onmousedown="msgw('aMessageastif1');"/>
		</p>
		</div>
		<div class="col-sm-1">
		<h:commandButton id="aBackToSearch1" immediate="true"  styleClass="btn btn-outline-dark" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetbldngpsq'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToPrevious_tt}" value="Voltar" action="#{cadAssetVhauxBean.doSelecionar}"/>
		</div>
	</div>
	</div>
...
<h:commandButton id="aBackToSearch1Menu" immediate="true"  styleClass="d-none" image="../imagens/btReturnxMenu.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetvhauxcad'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToMenu_tt}" action="#{cadAssetVhauxBean.doReturnMenu}"/>
<h:commandButton id="aUpdate" styleClass="d-none" rendered="#{cadAssetVhauxBean.canUpdate}" image="../imagens/btSave.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetvhauxcad'+'&ufmv='+stx;msgwr();upflag=0;AlternarTabsvf();" title="#{iimsg.botao_update}" action="#{cadAssetVhauxBean.doUpdate}"/>
<h:commandButton id="aCancelar" styleClass="d-none" rendered="#{cadAssetVhauxBean.canUpdate}" image="../imagens/btNew.gif"  disabled="" onmousedown="msgwr()" title="#{iimsg.botao_novo}" action="#{cadAssetVhauxBean.doCancelar}"/>
<h:commandButton id="aExcluir" styleClass="d-none" title="#{iimsg.botao_excluir}" rendered="#{cadAssetVhauxBean.canExcluir}" image="../imagens/btDelete.gif"  disabled="" onmousedown="msgwex()" action="#{cadAssetVhauxBean.doExcluir}"/>
<h:commandButton id="aRefresh" styleClass="d-none"  image="../imagens/btRefresh.gif" title="#{iimsg.botao_refresh}"  action="#{cadAssetVhauxBean.doObter}"  onmousedown="msgwr();document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetvhauxcad'+'&ufmv='+stx"/>
...
<h:commandButton id="aHelpAssetComRollsRfm" styleClass="d-none" tabindex="-1" immediate="true" rendered="#{cadAssetVhauxBean.hasHelpComRfm}" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes', 'AssetComRollsRfm');" image="../imagens/btHelpCom.gif"/>
<h:commandButton id="aHelpAssetRollsRfm" styleClass="d-none" tabindex="-1" immediate="true" rendered="#{cadAssetVhauxBean.hasHelpSpcRfm}" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes', 'AssetRollsRfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:inputHidden id="tabsvf" value="#{cadAssetVhauxBean.tabsvf}" />
<h:inputHidden id="xhelp_rc" value="#{cadAssetVhauxBean.xhelp_rc}" />
<h:inputHidden id="tabsvf_in" value="#{cadAssetVhauxBean.tabsvf_in}" />
<h:inputHidden id="tabsvf_tr" value="#{cadAssetVhauxBean.tabsvf_tr}" />
<h:inputHidden id="xbutn_id" value="#{cadAssetVhauxBean.xbutn_id}" />
<h:inputHidden id="xbutx_id" value="" />
<h:inputHidden id="xuserx_id_login" value="#{cadAssetVhauxBean.xuserx_id_login}" />
<h:inputHidden id="xmesg_rc" value="#{cadAssetVhauxBean.xmesg_rc}" />
<h:inputHidden id="xmesg_st" value="#{cadAssetVhauxBean.xmesg_st}" />
...
<h:outputText value="#{iimsg.AssetVhaux_D00_INFGR_UV_st}"/>
</h:outputLink>
</td>
...
<h:outputText value="#{iimsg.AssetVhaux_D00_CTB_UV_st}"/>
</h:outputLink>
</td>
...
<h:outputText value="#{iimsg.AssetVhaux_D00_STATUS_UV_st}"/>
</h:outputLink>
</td>
...
<iframe id="fInvtyVXsak" width="0" height="0" frameborder="0"> </iframe>
<iframe id="fInvtyVXsfk" width="0" height="0" frameborder="0"> </iframe>
<iframe id="InvtyVX" width="0" height="0" frameborder="0"> </iframe>
<iframe id="invty" width="0" height="0" frameborder="0"> </iframe>
<iframe id="fInvtyVXsak" width="0" height="0" frameborder="0"> </iframe>
<iframe id="fInvtyVXsfk" width="0" height="0" frameborder="0"> </iframe>
...
<h:commandLink id="xsel_vali" action="#{cadAssetVhauxBean.doValidate}" immediate="true"/>
...
<h:commandLink id="xsel_invtyvx" action="#{cadInvtyVXBean.doSelecionarOnly}" immediate="true"/>
