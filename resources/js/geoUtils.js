...
function MapLatLonToXY(phi, lambda, lambda0, xy) {
	...
	xy[0] = N
			* Math.cos(phi)
			* l
			+ (N / 6.0 * Math.pow(Math.cos(phi), 3.0) * l3coef * Math.pow(l,
					3.0))
			+ (N / 120.0 * Math.pow(Math.cos(phi), 5.0) * l5coef * Math.pow(l,
					5.0))
			+ (N / 5040.0 * Math.pow(Math.cos(phi), 7.0) * l7coef * Math.pow(l,
					7.0));
...
function converteLatLonParaUMT(_lat, _lon) {
	var resultado = [];	
	
	var xy = new Array(2);

	if (isNaN(parseFloat(_lon))) {
		alert('Informe uma longitude válida. Ex: -43.230190');
		return resultado;
	}
	...
	if ((lon < -180.0) || (180.0 <= lon)) {
		alert('A longitude informada está fora do intervalo de -180 até 180.');
		return resultado;
	}

	if (isNaN(parseFloat(_lat))) {
		alert('Informe uma latitude válida. Ex: -22.912152');
		return resultado;
	}
	...
	if ((lat < -90.0) || (90.0 < lat)) {
		alert('A longitude informada está fora do intervalo de -90 até 90.');
		return resultado;
	}
