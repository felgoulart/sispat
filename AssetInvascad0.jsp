...
arAbas[2] = new stAba('aba_astst','abc_astst','flg_astst','u','a','aba_astst');
arAbas[3] = new stAba('aba_astoc','abc_astoc','flg_astoc','u','a','aba_astoc');
...
<title><h:outputText value="#{cadAssetInvasBean.trans_tt}"/></title>
</head>
<body onkeydown="rk();"  onload="window.focus();loading();ufmvini();tran='assetinvascad';cadbeanx='cadAssetInvasBean'; loadPage(<%= request.getAttribute("pagina") %>); AlternarAbas(parent.cadbeanx, <%=(Object)session.getAttribute(("cadAssetInvasBeanx"))%>);AlternarSubAbas();">
...
<div id="loading" class="loading" align="center"><h:outputText id="label_wait" value="#{iimsg.msg_wait}"/></div>
	<div class="container expand-max bg-transparent mt-3">
	  <div class="row">
		...
		<div class="col-sm-10">
		<h2 class="title" style="font-size: 35px;"><h:outputText styleClass="h2 mt-3"  value="#{cadAssetInvasBean.trans_tt}"/></h2>
		
		<p class="subtitle" style="text-align:center;color:#505050">
			<h:outputText id="asset_id11" value="#{cadAssetInvasBean.asset_id}"  />
			<h:outputText value="#{iimsg.AssetBldng_asset_nu_tt}"/>
			<h:outputText id="asset_id_dsc11" value="#{cadAssetInvasBean.asset_nu}" />
		</p>
		<p style="text-align:center;">
		<h:inputText id="mesg_varastif1" rendered="#{cadAssetInvasBean.canUpdate}" value="#{cadAssetInvasBean.mesg_var}"  styleClass="txrr pr5 tb450r" tabindex="-1" readonly="true"/>
		<h:commandButton id="aMessageastif1" image="../imagens/btMessage_off.gif" styleClass="d-none" title="#{iimsg.botao_message}" rendered="#{cadAssetInvasBean.canUpdate}" action="#{cadAssetInvasBean.doValidate}" onmousedown="msgw('aMessageastif1');"/>
		</p>
		</div>
		<div class="col-sm-1">
		<h:commandButton id="aBackToSearch1" immediate="true"  styleClass="btn btn-outline-dark" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetbldngpsq'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToPrevious_tt}" value="Voltar" action="#{cadAssetInvasBean.doSelecionar}"/>
		</div>
	</div>
	</div>
...
<h:commandButton id="aBackToSearch1Menu" immediate="true"  styleClass="d-none" image="../imagens/btReturnxMenu.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetinvascad'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToMenu_tt}" action="#{cadAssetInvasBean.doReturnMenu}"/>
<h:commandButton id="aUpdate" styleClass="d-none" rendered="#{cadAssetInvasBean.canUpdate}" image="../imagens/btSave.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetinvascad'+'&ufmv='+stx;msgwr();upflag=0;AlternarTabsvf();" title="#{iimsg.botao_update}" action="#{cadAssetInvasBean.doUpdate}"/>
<h:commandButton id="aCancelar" styleClass="d-none" rendered="#{cadAssetInvasBean.canUpdate}" image="../imagens/btNew.gif"  disabled="" onmousedown="msgwr()" title="#{iimsg.botao_novo}" action="#{cadAssetInvasBean.doCancelar}"/>
<h:commandButton id="aExcluir" styleClass="d-none" title="#{iimsg.botao_excluir}" rendered="#{cadAssetInvasBean.canExcluir}" image="../imagens/btDelete.gif"  disabled="" onmousedown="msgwex()" action="#{cadAssetInvasBean.doExcluir}"/>
<h:commandButton id="aRefresh" styleClass="d-none"  image="../imagens/btRefresh.gif" title="#{iimsg.botao_refresh}"  action="#{cadAssetInvasBean.doObter}"  onmousedown="msgwr();document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'assetinvascad'+'&ufmv='+stx"/>
...
<h:commandButton id="aHelpAssetRfm" tabindex="-1" immediate="true" rendered="#{cadAssetInvasBean.hasHelpComRfm}" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes', 'AssetRfm');" image="../imagens/btHelpCom.gif"/>
<h:commandButton id="aHelpAssetTerrnRfm" tabindex="-1" immediate="true" rendered="#{cadAssetInvasBean.hasHelpSpcRfm}" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes', 'AssetTerrnRfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:inputHidden id="tabsvf" value="#{cadAssetInvasBean.tabsvf}" />
<h:inputHidden id="xhelp_rc" value="#{cadAssetInvasBean.xhelp_rc}" />
<h:inputHidden id="tabsvf_in" value="#{cadAssetInvasBean.tabsvf_in}" />
<h:inputHidden id="tabsvf_tr" value="#{cadAssetInvasBean.tabsvf_tr}" />
<h:inputHidden id="xbutn_id" value="#{cadAssetInvasBean.xbutn_id}" />
<h:inputHidden id="xbutx_id" value="" />
<h:inputHidden id="xuserx_id_login" value="#{cadAssetInvasBean.xuserx_id_login}" />
<h:inputHidden id="xmesg_rc" value="#{cadAssetInvasBean.xmesg_rc}" />
<h:inputHidden id="xmesg_st" value="#{cadAssetInvasBean.xmesg_st}" />
...
<h:outputText value="#{iimsg.AssetInvas_D00_INFGR_UV_st}"/>
</h:outputLink>
</td>
...
<h:outputText value="#{iimsg.AssetInvas_D00_CARAC_UV_st}"/>
</h:outputLink>
</td>
<td width="" id="aba_astoc" style="display: none">
<A href="#" onclick="msgwv('xsel_vali')"><img id="flg_astoc" class="d-none d-none" align="absmiddle"style="padding:5px; float:left" /></A>
<h:outputLink value="#" onclick="AlternarAbas(parent.cadbeanx, 3);" styleClass="mnt">
<h:outputText value="#{iimsg.AssetInvas_D00_OCUP_UV_st}"/>
</h:outputLink>
</td>
...
<%@include file="AssetInvascadastif.jsp"%>
</div>
<!-- UV-UV-Stubs/MV D00_CARAC_UV-->
<div id="abc_astst" style="display: none">
<%@include file="AssetInvascadastst.jsp"%>
</div>
<!-- UV-UV-Stubs/MV D00_OCUP_UV-->
<div id="abc_astoc" style="display: none">
<%@include file="AssetInvascadastoc.jsp"%>
</div>
<!-- D00_NOTR -->
...
<h:commandLink id="xsel_vali" action="#{cadAssetInvasBean.doValidate}" immediate="true"/>
