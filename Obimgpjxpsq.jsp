...
<link rel="stylesheet" href="../Obimgpjxpsq.css" type="text/css">
...
<body onload="window.focus();loadPage();loading();helpcd='ObimgpjxSfm';;tran='obimgpjxpsq'; vl='';cadbeanx='cadObimgpjxBean';<%session.setAttribute("tipo","obimgpjxpsq");%>">
<h:form id="cadForm">
...
<h:commandButton id="aPrev" styleClass="btp"  image="../imagens/btPgAnterior.gif" title="#{iimsg.botao_anterior}" rendered="#{cadObimgpjxBean.hasPrevPage}" action="#{cadObimgpjxBean.doPaginaAnterior}"/>
<h:commandButton id="aNext" styleClass="btp"  image="../imagens/btPgProxima.gif" title="#{iimsg.botao_proxima}" rendered="#{cadObimgpjxBean.hasNextPage}" action="#{cadObimgpjxBean.doProximaPagina}"/>
<h:outputText id="label_fill" style="display:none" value="#{iimsg.label_fill}" />
<h:outputText id="label_notfound" style="display:none" value="#{iimsg.label_notfound}" />
<h:outputText id="sys_openning_form" style="display:none" value="#{iimsg.sys_openning_form}" />
<h:commandButton id="aFiltrar" styleClass="btp"  image="../imagens/btFilter.gif" title="#{iimsg.botao_filtrar}" action="#{cadObimgpjxBean.doSelecionar}"/>
<h:commandButton id="aLimpar" styleClass="btp" rendered="#{cadObimgpjxBean.canPesquisar}" image="../imagens/btCancel.gif" title="#{iimsg.botao_limpar}" action="#{cadObimgpjxBean.doLimparPesq}"/>
...
<h:commandButton id="aHelpObimgpjxSfm" tabindex="-1" rendered="#{cadObimgpjxBean.hasHelpSpcSfm}" immediate="true" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes','ObimgpjxSfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:inputText id="mesg_var" value="#{cadObimgpjxBean.mesg_var}"  styleClass="txrr pr5 tb600r" tabindex="-1" readonly="true"/>
</td> <%-- <close td> msg --%>
<td> &nbsp&nbsp </td>
<td      width="50" > <%-- open w/ inputText 222--%>
<%-- outputLabel msg --%>
...
<%-- outputText --%>
<td    width="100" > <h:inputText id="codfqt" onfocus="this.blur()" value="#{cadObimgpjxBean.codfqt}" size="5" maxlength="5"  styleClass="txl pl5 tbxr"/>
</td> <%-- <close td> msg --%>
</tr> <%-- <tr close> msg--%>
...
id="dtDados_tt">
<!-- propColumnGen -->
<h:column id="obimgpj_id">
<f:facet name="header">
<h:panelGroup id="pg_obimgpj_id_tt">
<h:outputText id="hd_obimgpj_id_tt_xide" title="#{iimsg.Obimgpj_obimgpj_id_tt}" value="#{iimsg.Obimgpj_obimgpj_id_tt}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:column id="obimgpj_id">
<f:facet name="footer">
<h:panelGroup id="fpg_obimgpj_id">
<h:inputText id="obimgpj_id_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObimgpjxBean.fil_obimgpj_id}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="fil_obimgpj_tp_cfpjtp_xfil" title="#{iimsg.hint_dblclick}" ondblclick="pw('C'+'TPPRJ', tran, this.id);" onblur="cfb(this.id, 'pfcfb9', 'pfcf9');" onfocus="this.className='pfcf9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObimgpjxBean.fil_obimgpj_tp_cfpjtp}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obimgpj_ds_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObimgpjxBean.fil_obimgpj_ds}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obimgpj_vc_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObimgpjxBean.fil_obimgpj_vc}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- recdWdColumnGen -->
...
<!-- Dados -->
<h:dataTable
value="#{cadObimgpjxBean.pesqData}"
var="Obimgpj"
binding="#{cadObimgpjxBean.pesqTable}"
rowClasses="tre9, tro9"
headerClass="tbh9"
footerClass="tbf9"
id="dtDados"
rows="#{cadObimgpjxBean.pageLength}"
first="#{cadObimgpjxBean.currentLine}">
<!-- propColumnGen -->
<h:column id="obimgpj_id">
<h:panelGroup id="pg_obimgpj_id_sg">
<!-- propColumnGen String -->
<h:outputLabel id="sel_obimgpj_id"  onclick="slvl(this.id,this.value);" value="#{Obimgpj.obimgpj_id}"/>
...
<h:commandLink id="flag_imgObimgpj" onmousedown="wtran='obimgpjcad';el('hiddenform').target=tran;aa=this.id.slice(0,this.id.search('flag_img'));valor1=el(aa+'sel_obimgpj_id').innerHTML;valor2=el(aa+'sel_obimgpj_sq').value;el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;showpopup(tran,'xrec_obob',this.id,wtran,'yes');">
...
<!-- BBB--> <h:commandLink id="flag_imgrecd_wd" action="#{cadObimgpjxBean.doObternullSearchBack}" onmousedown="wtran='obimgpjxcad';aa=this.id.slice(0,this.id.search('flag_img'));valor1=rpel(aa+'sel_obimgpj_id');valor2=rpel(aa+'sel_obimgpj_sq');selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;">
...
<h:inputHidden id="sel_obimgpj_sqpk" value="#{Obimgpj.obimgpj_sq}"/>
<!-- 111--> <h:commandLink id="flag_imgrecd_wdpk" action="#{cadPjausxBean.doObternull}" onmousedown="openningForm();el('hiddenform').target=tran;aa=this.id.slice(0,this.id.search('flag_img'));valorPKey=rpel(aa+'sel_obimgpj_id');typevl=rpel(aa+'sel_obimgpj_tp_cfpjtp');tranx=PjausType(typevl);wtran=tranx+'cad';selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valorPKey;showpopup(tran,'xrec_'+tranx,this.id,wtran,'yes');">
...
<h:commandLink id="xrec_obnv" action="#{cadObimgpjxBean.doObterParentForCancel}" immediate="true"/>
