...
String pkvl = ""; String dsvl = ""; String akvl = "";
String jspk = ""; String jsds = ""; String jsak = "";

if ( ch.equals("") ) {
    ch = "x";
    jspk="cadForm:"+ph+fdpk;
    jsds="cadForm:"+ph+fdak+"_desc";
    jsak="cadForm:"+ph+fdak;
} else {
    jsds="cadForm:tab"+ch+":"+fdak+"_desc0";
    jspk="cadForm:tab"+ch+":"+fdpk;
    jsak="cadForm:tab"+ch+":"+fdak;
}
int vflag = 0;
String desc = "";
...
if (vflag == 0) {
    ...
    try {
        if (vl != null) {
            ...
            List list = daox.find(con, ak, vl);
            if (list.isEmpty()) {
                dsvl = "sys_notfound";
            } else {
                dsvl = ((DataObject)list.get(0)).getString(ds);
                pkvl = ((DataObject)list.get(0)).getString(pks);
                akvl = ((DataObject)list.get(0)).getString(aks);
            }
        }
    } finally {
...
<script  type="text/javascript"  >
    ...
    upflag=0;
    parent.el('fiiBv').src = 'iiBv.jsp?cb='+parent.cadbeanx+'&fd='+'<%= fdak %>'+'&ch=<%= ch %>'+'&nb=<%= st %>&vf=<%=_vf%>&vl='+parent.rpeln('<%= jsak %>')+'';
</script>
</html>
