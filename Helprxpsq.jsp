...
<body onload="window.focus();loadPage();loading();helpcd='HelprxSfm';;tran='helprxpsq'; vl='';cadbeanx='cadHelprxBean';<%session.setAttribute("tipo","helprxpsq");%>">
<h:form id="cadForm">
...
<h:commandButton id="aBackToSearchMenu" styleClass="btp" image="../imagens/btReturnxMenu.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'helprxcad'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToMenu_tt}" action="#{cadHelprxBean.doReturnMenu}"/>
<h:commandButton id="aPrev" styleClass="btp"  image="../imagens/btPgAnterior.gif" title="#{iimsg.botao_anterior}" rendered="#{cadHelprxBean.hasPrevPage}" action="#{cadHelprxBean.doPaginaAnterior}"/>
<h:commandButton id="aNext" styleClass="btp"  image="../imagens/btPgProxima.gif" title="#{iimsg.botao_proxima}" rendered="#{cadHelprxBean.hasNextPage}" action="#{cadHelprxBean.doProximaPagina}"/>
<h:outputText id="label_fill" style="display:none" value="#{iimsg.label_fill}" />
<h:outputText id="label_notfound" style="display:none" value="#{iimsg.label_notfound}" />
<h:outputText id="sys_openning_form" style="display:none" value="#{iimsg.sys_openning_form}" />
<ps:psCommandLink id="aFiltrar" styleClass="btp"  ativo="../imagens/btFilter.gif" title="#{iimsg.botao_filtrar}" action="#{cadHelprxBean.doSelecionar}">
<f:param name="campo9" value=""/>
<f:param name="tipo" value="helprxpsq"/>
</ps:psCommandLink>
<h:commandButton id="aLimpar" styleClass="btp" rendered="#{cadHelprxBean.canPesquisar}" image="../imagens/btCancel.gif" title="#{iimsg.botao_limpar}" action="#{cadHelprxBean.doLimparPesq}"/>
...
<h:commandButton id="aHelpHelprxSfm" tabindex="-1" rendered="#{cadHelprxBean.hasHelpSpcSfm}" immediate="true" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes','HelprxSfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:inputText id="mesg_var" value="#{cadHelprxBean.mesg_var}"  styleClass="txrr pr5 tb600r" tabindex="-1" readonly="true"/>
</td> <%-- <close td> msg --%>
<td> &nbsp&nbsp </td>
<td      width="50" > <%-- open w/ inputText 222--%>
<%-- outputLabel msg --%>
...
<%-- outputText --%>
<td    width="100" > <h:inputText id="codfqt" onfocus="this.blur()" value="#{cadHelprxBean.codfqt}" size="5" maxlength="5"  styleClass="txl pl5 tbxr"/>
</td> <%-- <close td> msg --%>
</tr> <%-- <tr close> msg--%>
...
<h:commandLink id="hoord_helpr_sq" action="#{cadHelprxBean.doOrd_helpr_sq}">
<ps:psGraphicImage id="hord_helpr_sq_desc" url="../imagens/btOrder_up.gif" border="0" rendered="#{cadHelprxBean.ord_helpr_sq_desc}"/>
<ps:psGraphicImage id="hord_helpr_sq" url="../imagens/btOrder_down.gif" border="0" rendered="#{cadHelprxBean.ord_helpr_sq}"/>
...
<h:inputText id="tabs_vc_xfil" onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadHelprxBean.fil_tabs_vc}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- recdWdColumnGen -->
...
<!-- Dados -->
<h:dataTable
value="#{cadHelprxBean.pesqData}"
var="Helpr"
binding="#{cadHelprxBean.pesqTable}"
rowClasses="tre9, tro9"
headerClass="tbh9"
footerClass="tbf9"
id="dtDados"
rows="#{cadHelprxBean.pageLength}"
first="#{cadHelprxBean.currentLine}">
<!-- propColumnGen -->
<h:column id="helpr_sq">
...
<h:commandLink id="flag_imgrecd_wd" action="#{cadHelprxBean.doObternullSearchBack}" onmousedown="wtran='helprxcad';aa=this.id.slice(0,this.id.search('flag_img'));valor1=rpel(aa+'sel_helpr_cd_help');valor2=rpel(aa+'sel_helpr_lg_help');valor3=rpel(aa+'sel_helpr_cd_help_rel');selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2+','+valor3;">
...
<h:inputHidden id="sel_helpr_cd_helppk" value="#{Helpr.helpr_cd_help}"/>
<h:inputHidden id="sel_helpr_lg_helppk" value="#{Helpr.helpr_lg_help}"/>
<h:commandLink id="flag_imgrecd_wdpk" action="#{cadHelpmaintxBean.doObternull}" onmousedown="el('hiddenform').target=tran;aa=this.id.slice(0,this.id.search('flag_img'));valorPKey=rpel(aa+'sel_helpr_lg_help');typevl=rpel(aa+'sel_');tranx=HelpmaintType(typevl);wtran=tranx+'cad';selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valorPKey;showpopup(tran,'xrec_'+tranx,this.id,wtran,'yes');">
...
<h:commandLink id="xrec_obnv" action="#{cadHelprxBean.doObterParent}" immediate="true"/>
