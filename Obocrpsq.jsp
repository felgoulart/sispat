...
<link rel="stylesheet" href="../Obocrpsq.css" type="text/css">
...
<body onload="window.focus();loadPage();loading();helpcd='ObocrSfm';;tran='obocrpsq'; vl='';cadbeanx='cadObocrBean';<%session.setAttribute("tipo","obocrpsq");%>">
<h:form id="cadForm">
...
<h:commandButton id="aBackToSearchMenu" styleClass="btp" image="../imagens/btReturnxMenu.gif" onmousedown="if(abatp=='m'){ufmsg1();return;};if(keypress==1){ufmsg2();return;};document.getElementById('iiSaveUfmv').src = 'iiSaveUfmv.jsp?tipo='+'obocrcad'+'&ufmv='+stx;msgwr();upflag=0;" title="#{iimsg.buttonReturnToMenu_tt}" action="#{cadObocrBean.doReturnMenu}"/>
<h:commandButton id="aPrev" styleClass="btp"  image="../imagens/btPgAnterior.gif" title="#{iimsg.botao_anterior}" rendered="#{cadObocrBean.hasPrevPage}" action="#{cadObocrBean.doPaginaAnterior}"/>
<h:commandButton id="aNext" styleClass="btp"  image="../imagens/btPgProxima.gif" title="#{iimsg.botao_proxima}" rendered="#{cadObocrBean.hasNextPage}" action="#{cadObocrBean.doProximaPagina}"/>
<h:outputText id="label_fill" style="display:none" value="#{iimsg.label_fill}" />
<h:outputText id="label_notfound" style="display:none" value="#{iimsg.label_notfound}" />
<h:outputText id="sys_openning_form" style="display:none" value="#{iimsg.sys_openning_form}" />
<h:commandButton id="aFiltrar" styleClass="btp"  image="../imagens/btFilter.gif" title="#{iimsg.botao_filtrar}" action="#{cadObocrBean.doSelecionar}"/>
<h:commandButton id="aLimpar" styleClass="btp" rendered="#{cadObocrBean.canPesquisar}" image="../imagens/btCancel.gif" title="#{iimsg.botao_limpar}" action="#{cadObocrBean.doLimparPesq}"/>
<h:commandButton id="aNew" styleClass="btp" rendered="#{cadObocrBean.canInserir}" image="../imagens/btNew.gif" onmousedown="fNovorwpp('obocrcad', tran, this.id, 'yes', fPgAssist());" title="#{iimsg.botao_novo}" action="#{cadObocrBean.doObterParentForCancel}"/>
...
<h:commandButton id="aHelpObocrSfm" tabindex="-1" rendered="#{cadObocrBean.hasHelpSpcSfm}" immediate="true" title="#{iimsg.botao_help_spc}" onmousedown="pw('helpcad', tran, this.id, 'yes','ObocrSfm');" image="../imagens/btHelpSpc.gif"/>
...
<h:outputLabel  styleClass="tip txr pl0"  id="lbl_obocr_id" for="obocr_id"  title="#{iimsg.Obocr_obocr_id_tt}"  value="#{iimsg.Obocr_obocr_id_tt}"/>  </td> <%-- <top> --%>
<td      width="400" >
<h:inputText id="obocr_id" tabindex="-1" readonly="true" value="#{cadObocrBean.obocr_id}" size="20" maxlength="20" onkeydown="keypress=1;"  styleClass="tbxr"/>
<h:commandLink id="obocr_id_desc" value="#{cadObocrBean.obocr_id_desc}" styleClass="dsp"  onmousedown="pwfk('assetcad', tran, this.id, 'no')"/>
</td> <%-- <close td> pKey --%>
</td> <%-- <close td> Button --%>
</tr> <%-- <tr close> pKey--%>
...
<h:inputText id="mesg_var" value="#{cadObocrBean.mesg_var}"  styleClass="txrr pr5 tb600r" tabindex="-1" readonly="true"/>
</td> <%-- <close td> msg --%>
<td> &nbsp&nbsp </td>
<td      width="50" > <%-- open w/ inputText 222--%>
<%-- outputLabel msg --%>
...
<%-- outputText --%>
<td    width="100" > <h:inputText id="codfqt" onfocus="this.blur()" value="#{cadObocrBean.codfqt}" size="5" maxlength="5"  styleClass="txl pl5 tbxr"/>
</td> <%-- <close td> msg --%>
</tr> <%-- <tr close> msg--%>
...
<h:inputText id="fil_obocr_tp_acao_xfil" title="#{iimsg.hint_dblclick}" ondblclick="pw('C'+'TPACO', tran, this.id);" onblur="cfb(this.id, 'pfcfb9', 'pfcf9');" onfocus="this.className='pfcf9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObocrBean.fil_obocr_tp_acao}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obocr_cd_doc_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObocrBean.fil_obocr_cd_doc}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- codfColumnGen -->
...
<h:inputText id="fil_obocr_tp_status_xfil" title="#{iimsg.hint_dblclick}" ondblclick="pw('C'+'STOCP', tran, this.id);" onblur="cfb(this.id, 'pfcfb9', 'pfcf9');" onfocus="this.className='pfcf9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObocrBean.fil_obocr_tp_status}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="obocr_ds_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObocrBean.fil_obocr_ds}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- codfColumnGen -->
...
<h:inputText id="fil_obocr_tp_cfasttp_xfil" title="#{iimsg.hint_dblclick}" ondblclick="pw('C'+'ASTTP', tran, this.id);" onblur="cfb(this.id, 'pfcfb9', 'pfcf9');" onfocus="this.className='pfcf9'"  onchange="this.value = this.value.toUpperCase();" onclick="this.value=''" value="#{cadObocrBean.fil_obocr_tp_cfasttp}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- propColumnGen -->
...
<h:inputText id="tabs_vc_xfil" onchange="this.value = this.value.toUpperCase();" ondblclick="this.value=''" value="#{cadObocrBean.fil_tabs_vc}" onkeypress="if (event.keyCode == 13) { el('cadForm:aFiltrar').click();}"/>
</h:panelGroup>
</f:facet>
</h:column>
<!-- recdWdColumnGen -->
...
<!-- Dados -->
<h:dataTable
value="#{cadObocrBean.pesqData}"
var="Obocr"
binding="#{cadObocrBean.pesqTable}"
rowClasses="tre9, tro9"
headerClass="tbh9"
footerClass="tbf9"
id="dtDados"
rows="#{cadObocrBean.pageLength}"
first="#{cadObocrBean.currentLine}">
<!-- propColumnGen -->
<h:column id="obocr_dt">
...
<h:commandLink id="flag_imgObocr" onmousedown="wtran='obocrcad';el('hiddenform').target=tran;aa=this.id.slice(0,this.id.search('flag_img'));valor1=el(aa+'sel_obocr_id').value;valor2=el(aa+'sel_obocr_sq').value;el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;showpopup(tran,'xrec_obob',this.id,wtran,'yes');">
...
<h:inputHidden id="sel_obocr_id" value="#{Obocr.obocr_id}"/>
<h:inputHidden id="sel_obocr_sq" value="#{Obocr.obocr_sq}"/>
<!-- BBB--> <h:commandLink id="flag_imgrecd_wd" action="#{cadObocrBean.doObternullSearchBack}" onmousedown="wtran='obocrcad';aa=this.id.slice(0,this.id.search('flag_img'));valor1=rpel(aa+'sel_obocr_id');valor2=rpel(aa+'sel_obocr_sq');selDtTableOneRow(aa);el('fiiSaveType').src = 'iiSaveType.jsp?tipo='+wtran+'&valor='+valor1+','+valor2;">
...
<h:commandLink id="xrec_obnv" action="#{cadObocrBean.doObterParentForCancel}" immediate="true"/>
