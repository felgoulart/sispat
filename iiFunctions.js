enablePopup = false;/* Flag que liga/desliga o recurso de popup. Por Wendel Silverio <wendelsilverio@gmail.com> */
...
screenHeigthcm=22.7; screenHeightResol = 768, verticalPixRate = screenHeightResol / screenHeigthcm;
screenWidthcm=30; screenWidthResol = 1024, horizontalPixRate = screenWidthResol / screenWidthcm;
...
function goBack() {
    alert('Retorne a Pagina Anterior');  
    document.getElementById("myBtn").addEventListener("click", window.history.go(-1), true);
    //document.getElementById("myBtn").addEventListener("mousedown", window.history.go(-1), true);
    //window.history.go(-1);
    //window.history.back();
}
...
function slvl(cl) { // Busca Valid Codf
	vl = rel(cl);
	kk = cl.indexOf("sel_")+4;
    id = cl.substring(kk,cl.length);
	fil = "cadForm:dtDados_ft:"+id+"_xfil";
    //alert("fil: "+fil+" vl: "+vl);
	el(fil).value = vl;
    //	cadForm:dtDados:X:sel_asset_km_ini
    //	cadForm:dtDados_ft:sel_asset_km_ini_xfil
}
function slvlprm(cl) { // Busca Valid Codf
	vl = rel(cl);
	kk = cl.indexOf("sel_")+4;
    id = cl.substring(kk,cl.length);
	fil = "cadForm:p_"+id+"";
    ...
    vlzz = rel(fil);
    vl = vl.trim();
    startzz = vlzz.indexOf(vl+' '); 
    if ( startzz < 0 ) {
        //alert("startzz: "+startzz+"vlzz: "+vlzz+" vl: "+vl);
    	vlzz = vlzz + ' ' + vl;  //vlx = vlx.trim;
    	vlzz = vlzz.trim();
    	el(fil).value = vlzz;
    }
...
function slvlcdprm(cl) { // Busca Valid Codf
	vl = rel(cl);
	kk = cl.indexOf("sel_")+4;
    id = cl.substring(kk,cl.length);
	fil = "cadForm:"+"p_"+id+"";
...
function slvlcd(cl) { // Busca Valid Codf
	vl = rel(cl);
	kk = cl.indexOf("sel_")+4;
    id = cl.substring(kk,cl.length);
	fil = "cadForm:dtDados_ft:"+"fil_"+id+"_xfil";
    //alert("fil: "+fil+" vl: "+vl);
	el(fil).value = vl;
	//	cadForm:dtDados:X:sel_asset_km_ini
    //	cadForm:dtDados_ft:sel_asset_km_ini_xfil
...
function Transfkx(ph, ch, st, fdpk, dao, pk, ds, vl) { // Search & Validate Alternate Foreing Key Hidden
    uf();
    vl = vl.toUpperCase();
    if (ch != '') {
        vf = valueVf('tab'+ch+':'+ch+'_vf0');
        el('fiiTransfkx').src = 'iiTransfkx.jsp?ph='+ph+'&ch='+ch+'&st='+st+'&fdpk='+fdpk+'&dao='+dao+'&pk='+pk+'&ds='+ds+'&vl='+vl+'&vf='+vf;
    } else {
        el('fiiTransfkx').src = 'iiTransfkx.jsp?ph='+ph+'&ch='+ch+'&st='+st+'&fdpk='+fdpk+'&dao='+dao+'&pk='+pk+'&ds='+ds+'&vl='+vl;
    }
}
...
function openningForm() { // Alert - Delete Confirmation
    alert(rel('cadForm:sys_openning_form'));
}
...
function msgConfMnhr() { // Alert - Delete Confirmation
    var stxx1 = rel('cadForm:crwop_st_cfcrwst_a_var');
...
function msgConfMnhrSt() { // Alert - Delete Confirmation
    ...
    if (stxx.indexOf("BJ") >= 0 ||  stxx.indexOf("EJ") >= 0   ) {
    	//msgConfMnhrSt_conf()
    	return false;
    } else {
        return true;
...
msgwinclose = function() { // Closes Current Window and if exists an associated Message Window
	if(enablePopup) {
        alert('enablePopup');
		/* 
    	 * Código candidato a ser removido em versões posteriores 
    	 * Wendel Silvério <wendelsilverio@gmail.com>
    	 */
		if (winId !== null){winId.close();}
		if (winIdx !== null){winIdx.close();}
		window.close();
	} else {
		var modal = parent.document.getElementById('codfModal');
...
function pwcodfms(cd, tr, target, scroll, value) { // Busca Valid - Multivalued With Divs (chlds)  
	//alert('cdms: '+cd);  
	//if (cd == 'C') { return;}
	if (cd.indexOf("noapply") >= 0) {
        alert(rel('cadForm:msg_itDoesntApply'));   
...
function showpopup(pwin, submit, target, wtran, scroll) {  //  Web Window
    ...
    if(enablePopup) { 
    	...
    	/* Código para popup via DIV
    	 * por Wendel Silverio <wendelsilverio@gmail.com>
    	 */
    	var codfModal = document.getElementById('codfModal');
    	var codfModalContent = document.getElementById('codfModalContent');
    	var codfPopup = document.getElementById('codfPopup');
    	
    	var OFFSET_HEIGHT = 130;
    	var OFFSET_WIDTH = 34;
    	
    	var height = parseInt(h2,10) + OFFSET_HEIGHT;
    	var width = parseInt(w2,10) + OFFSET_WIDTH;
    	
    	/* 1,5% de margens */
    	var heightOffset = parseInt(window.screen.availHeight * 0.15);
    	var widthOffset = parseInt(window.screen.availWidth * 0.15);
    	
    	maxHeight = window.screen.availHeight - heightOffset;
    	maxWidth = window.screen.availWidth - widthOffset;
        //alert ("maxHeight..:"+maxHeight);  // 616
        //alert ("maxWidth..:"+maxWidth);    // 1162
    	if (submit.indexOf('codf') > 0) {
    		widthOffset = parseInt(900 * 0.15);
    		maxWidth = 900 - widthOffset;
    		//maxWidth = 700; // esconde o botão de fechamento X
    		maxWidth = 800;
    		//alert ("maxWidth..:"+maxWidth);
    	}	
    	
		codfModal.style.display = "block";
		
		codfModalContent.style.height=maxHeight;
    	codfModalContent.style.width=maxWidth;
        //alert ("fnx submit2..:"+submit);
        //alert ("fnx target2..:"+target);

    	codfPopup.src = '?popup=true&hiddenform:target='+target+'&hiddenform:source=cadForm:&hiddenform:sourcedesc=cadForm:nome&hiddenform=hiddenform&hiddenform:_idcl=hiddenform:'+submit.toLowerCase();
    	codfPopup.height = maxHeight;
    	codfPopup.width = maxWidth;
    	document.forms[1]['hiddenform:target'].value = target;
...
function showpopup55(pwin, submit, target, wtran, scroll) {  //  Web Window
    // Information Insights Copyright (October 2005)
    // submit.. Component of the hiddenform to be submitted
    // target.. Name of the Target Component receipient of the value / description)
    //  This function calls the popup window.
    ...
    features="height="+h2+",width="+w2+",status=yes,resizable=yes,toolbar=no,menubar=no,scrollbars="+scroll+", location=no, offscreenBuffering=true, left="+xleftppw+", top="+ytop;
    // xxx winIdx=window.open('',xwindow,features); // open an empty window
    //a = winIdx.parent.name;
    //alert('a..:'+a);
    //hform=parent.document.forms['hiddenform']; // reference to the hidden form
    //alert('hform..:'+hform);
...
// =========================================================================================
function onclickx(submit, scroll) {  //  Web Window
    if (scroll != null) {
        if (scroll.toLowerCase == 'yes') { scroll == 'yes'; }
        else { scroll == 'no'; }
    } else { scroll == 'no'; }
    submit = submit.toLowerCase();
    //alert('submit' +submit);
    parent.document.getElementById('cadForm:'+submit).onclick();  
}
// =========================================================================================
...
function pel(id) {
  var result = parent.document.getElementById(id);
  if(result == null) {
...
function validaDatas2() { //recebe como argumentos os objetos dos campos de data
  valx = validaDatas2.arguments[0].value;
  //alert(valx);
  //if (vl == 'NAP' || vl == 'NPI'  ) {
  //	return;
  //}	
...
function valDec(value) {
  ...
  if (value.indexOf('0,00') == 0 ) {
	  alert(' >>> Digite no formato 9999 para que ZEROS e separadores de milhar serão colocados automaticamente, ou no formato 9999,99 (com casas decimais explicitas! ') 
	  return '';
  }
  if(/^-?\d+(\,\d+)?$/.test(value)) {
    return value;
...
function selOne(value) {
	//alert('value: '+value); 
	posx = value.indexOf('>');
	if (posx < 0) {value = '>'+value;}
	if (posx >= 0) {
		value = value.replace(">", "");
		posx = value.indexOf(',',posx+1);
        if (posx < 0) {
        	value = '>'+value;
        } else {	
		    value1 = value.substring(0,posx+2);
		    value2 = value.substring(posx+2,value.length);
		    value =  value1+'>'+value2;
        }
    }
	return value;
...
function valDecx(value) {
  ...
  if(/^-?\d+(\,\d+)?$/.test(value)) {
	  ...
	  if (value.indexOf(',') > 0 ) { 
          ...
          ll = value.length - cp; 
          //alert('ll: '+ll);
          if (cp == 2 ) { value = '00'+value;} else	  
          if (cp == 3 ) { value = '0'+value;}   
          if (ll == 2) {value = value + '0';} else
          if (ll == 1) {value = value + '00';}
	  } else
	  if (value.indexOf(',') < 0) { 	  
	      if (value.length == 1) { value = '00'+value + ',000';} else
	      if (value.length == 2) { value = '0'+value + ',000';} else
	      if (value.length == 3) { value = value + ',000';} else
	      if (value.length == 4) { 
              value1 = value.substring(0,3);
              value2 = value.substring(3,4);
              value = value1+','+value2+'00';
	      } else
	      if (value.length == 5) { 
              value1 = value.substring(0,3);
              value2 = value.substring(3,5);
              value = value1+','+value2+'0';
	      } else
	      if (value.length >= 6) { 
              value1 = value.substring(0,3);
              value2 = value.substring(3,6);
              value = value1+','+value2;
	      }
	  }
	  //if (value.length == 5) { value = value + '00';}
	  //if (value.length == 6) { value = value + '0';}
	  return value;
  } else {
    alert(' >>> Digite no formato 9,9 que ZEROS serão colocados automaticamente. Mas se não digitar Virgula, VÍRGULA e ZEROS serão colocados automaticamente no formato 999,999 !') 
  //if(/^-?\d+(\.\d+)?$/.test(value)) {
  //    return value;;
  //} else {
    return '';;
...
function ckbSelIns__a(checkbox) {
  ...
  if (val == 'false') {
    ...
    // hoverDatatableRow
    if (chk_w_flag) {
       setSelectedRow(ins_a,dataTable);
     }
...
function ckbSelListx__a(column) {
    flag = true;
    i = 0;
    sellist = 'cadForm:selection_list_var';
    pathx = 'cadForm:dtDados:';
...
function ckbSelListz__a(column) {
    flag = true;
    i = 0; sepx = "";
    sellist = 'cadForm:selection_list_var';
    pathx = 'cadForm:dtDados:';
    ...
    while (flag) {
      ...
      if (chk_w == 'true') {
          valx = rel(seqw);
          if (valx.indexOf('Inexistente') >= 0 || valx.indexOf('Não Informado') >= 0 || valx.indexOf('Não se aplica') >= 0 ) { 
              listx = valx;
              flag = false; 
              continue;
          } else {
        	  sepx = ' ';
        	  if (valx.indexOf('a') >= 0) {sepx = '; ';}
        	  if (valx.indexOf('e') >= 0) {sepx = '; ';}
        	  if (valx.indexOf('i') >= 0) {sepx = '; ';}
        	  if (valx.indexOf('o') >= 0) {sepx = '; ';}
        	  if (valx.indexOf('u') >= 0) {sepx = '; ';}
        	  listx = listx + valx + sepx;
          }
          //alert('listx: '+listx);
      } 	  
      i++;
    ...
    if (listx.indexOf(",") >= 0 ) {
        listx = listx.substring(0, listx.lastIndexOf(","));
    }
    listx = listx.replace(/(\r\n\t|\n|\r\t)/gm,"");
   // alert(listx);
    if_el('cadForm:selection_list_var', listx);
...
function ckbContOrDisc() {
	//rrr = confirm("Atención >> Para Seleccinar en Bloque Vehúculos en la misma Situacion, Pressione <Ok> ! Para Seleccionar de forma Intercalada Vehículos en la misma situacion, Pressione <Cancel>");
	rrr = confirm(rel('cadForm:msg_selectBlockSelectionOption'));
}
function ckbSelCont__a(checkbox) {
  ...
  vhtype= rel(path_in + number + ':sel_vhcle_tp__a');
  vhtype = vhtype.substring(0,vhtype.indexOf("/"));
  if_el('cadForm:vhcle_tp_a_var', vhtype);
  //alert(vhtype);
  //alert(numbsq);
  ...
  if (rel('cadForm:pos_ins_pos_ckb_a_var') == 'true') {
    ...
    if (val == 'false') {
      ...
      setSelectedRow(number,dataTable);
...
function ckbSelContSt__a(checkbox) {
  id = checkbox.id;
  //alert('id: '+id);
  val = rel(checkbox.id);
  chk_path = id.slice(0, id.lastIndexOf(":"));
  ...
  if (rel('cadForm:pos_ins_pos_ckb_a_var') == 'true') {
    ...
    for (j = ini_a; j < fin_for; j++) {
      ...
      if_bel(chk_w, 'true');
      if ( inix == 0) {
          st1 = rel(path_in + j + ':sel_vhopr_cd_situation__a'), rel(path_in + j + path_ot);
          inix = 1;
      }	
      stx = rel(path_in + j + ':sel_vhopr_cd_situation__a'), rel(path_in + j + path_ot);
      //alert ('j: '+j+'st1: '+st1+' stx:'+stx);
      if ( inix > 0 && st1 != stx && difx == 0 ) {
          //alert ('j: '+j+'st1: '+st1+' stx:'+stx);
          //alert('ini_a: '+ini_a);
          //alert('fin_for: '+fin_for);
    	  //alert ('Mo es permitido seleccionar vehiculos en situaciones distintas para hacer cambio en bloque');
    	  alert (rel('cadForm:msg_dontSelectVehiclesInDiffStatus'));
    	  difx= 1;
      } 	  
    ...
    if (difx == 1) {
        ...
        for (j = ini_a; j < fin_for; j++) {
          ...
          if_el('cadForm:pos_fin_a_var', fin_a);
          if_el('cadForm:dtDados_tt:chk_sel__a_tt', 'false');
          //trs[j].className = originalClass[j];
          //trs[j].onmouseover = function() { this.className='trh9' };
          //trs[j].onmouseout = function() { this.className=originalClass[j] };
        }
        return;
    }
...
function ckbSelContIx__a(checkbox) {
	if (strbtn.indexOf('VhcleQueryListparm_a') >= 0 ) {
        //alert('Selecione un boton de Operación para hacer una seleción');
        alert (rel('cadForm:msg_selectAnOperationButton'));
        if_el(checkbox.id, 'false');
	...
	id = checkbox.id;
	  ...
	  number = number.slice(number.lastIndexOf(":")+1,number.length);
	  //numbsq= rel(path_in + number + ':sel_vhlst_sq_vhcle__a');
	  //alert('number sliced: '+number);
	  //numbsq= rel(path_in + number + ':sel_vhlst_sq_vhcle__a');
	  numbsq = parseInt(number); numbsq = numbsq + 1;
	  //alert(number);
	  //alert(numbsq);
	  ...
	  if (rel('cadForm:pos_ins_pos_ckb_a_var') == 'true') {
	    ...
	    while (flag) {
	      //ckbSelmaplist__a(rel(path_in + i + ':sel_vhlst_sq_vhcle__a'), rel(path_in + i + path_ot));
	      ckbSelIxmaplist__a(i+1, rel(path_in + i + path_ot));
	      chk_w = path_in+i+path_ot;
	      i++;
	      if (el(path_in+i+path_ot)) {
	        flag = true;
...
function ckbSelIxmaplist__a(nsq, vlc) {
	  //alert('nsq: '+nsq);alert('vlc: '+vlc);
	  if_el('cadForm:dtDados_tt:chk_sel__a_tt', 'false');
	  el('cadForm:dtDados_tt:chk_sel__a_tt').style.background="#FFE4D0";
...
function ckbSelAll(checkbox) {
	...
	path_ot = path_ot.replace('_tt', '');
	//alert('path_ot: '+path_ot);
	el('cadForm:dtDados_tt:chk_sel_tt').style.background="";
...
function ckbSelIns__b(checkbox) {
  ...
  if (val == 'false') {
    ...
    // hoverDatatableRow
    if (chk_w_flag) {
       setSelectedRow(ins_b,dataTable);
     }
...
function ckbSelDisc__b(checkbox) {
  ...
  if (rel('cadForm:pos_ins_pos_ckb_b_var') == 'true') {
    ...
    if (val == 'true') {
        ...
        // Handle Vehicle Type Selection ------------------------
        if(el('cadForm:vhcle_tp_b_var')) { // Get Selected Type
            ini_b_tp = rel('cadForm:vhcle_tp_b_var');
            ini_b_tp_desc = el('cadForm:vhcle_tp_b_var_desc').title;
        }
        // Get table b Selected Type
        //alert('checkbox.id: '+checkbox.id);
	    start = checkbox.id.indexOf('dos1:')+5; end = checkbox.id.indexOf(':chk');
		pos = checkbox.id.substring(start,end);
		//alert('pos: '+pos);
	    ini_b_tp_tab = rel(path_in + pos + ':sel_vhcle_tp__b');
	    ini_b_tp_tab_desc = el(path_in + pos + ':sel_vhcle_tp__b').title;
	    if (ini_b_tp_tab.indexOf('/') >= 0) {
	        ini_b_tp_tab = ini_b_tp_tab.substring(0,ini_b_tp_tab.indexOf('/')); 
		}
	    //alert('bb ini_b_tp_tab: '+ini_b_tp_tab+' ini_b_tp: '+ini_b_tp );
        // If selected vehicle type is dif from selected type abort 
	    if(ini_b_tp_tab != ini_b_tp && ini_b_tp.length > 0  ) {
	        dif_ini_b_tp = true;
	        if_el(checkbox.id, 'false');
	        //if_el('cadForm:pos_ini_b_var', ini_b);
		    alert(rel('cadForm:sys_dif_chkbox'));
        ...
        if_el('cadForm:vhcle_tp_b_var', ini_b_tp_tab);
        if_el('cadForm:vhcle_tp_b_var_desc', ini_b_tp_tab_desc);
        if_el('cadForm:codf_id_vhcst_b_var', '');
        if_el('cadForm:codf_id_vhcst_b_var_desc', '');
	    // ------------------------------------

        setSelectedRow(number,dataTable);

        //alert(' before number: '+number+' ini_b: '+ini_b);
      ...
      //alert(' after number: '+number+' ini_b: '+ini_b+' fin_b: '+fin_b);
      if (parseInt(number) > parseInt(ini_b)) {
        fin_b = number -1;
      ...
      // Handle Start / End Positions & Vhc Type Cleaning -------------------------- 
      fin_bx = rel('cadForm:pos_fin_b_var');
      fin_for = parseInt(fin_bx); flagsel = 0;
      for (j = 0 ; j <= fin_for; j++) { // Check if checkbox is marked
          //alert('path_in+j+path_ot: '+path_in+j+path_ot+' vl: '+rel(path_in+j+path_ot)+' j: '+j+'fin_for: '+fin_for+'flagsel: '+flagsel);
          if (rel(path_in+j+path_ot) == 'true') {
              flagsel = 1;
    ...
    //if (strbtn.indexOf('tatusUpdate_b') < 0 ) {
        //chbSelType__b(checkbox);
    //}
  }
}
...
function ckbSelContSt__b(checkbox) {
  id = checkbox.id;
  val = rel(checkbox.id);
  chk_path = id.slice(0, id.lastIndexOf(":"));
  //alert(chk_path);
  ...
  if (rel('cadForm:pos_ins_pos_ckb_b_var') == 'true') {
    ...
    for (j = ini_b; j < fin_for; j++) {
      ...
      if ( inix == 0) {
          st1 = rel(path_in + j + ':sel_vhopr_cd_situation__b'), rel(path_in + j + path_ot);
          inix = 1;
      }	
      stx = rel(path_in + j + ':sel_vhopr_cd_situation__b'), rel(path_in + j + path_ot);
      //alert ('j: '+j+'st1: '+st1+' stx:'+stx);
      if ( inix > 0 && st1 != stx && difx == 0 ) {
          //alert ('j: '+j+'st1: '+st1+' stx:'+stx);
          //alert('ini_a: '+ini_a);
    ...
    if (difx == 1) {
        ...
        for (j = ini_b; j < fin_for; j++) {
          ...
          if_el('cadForm:pos_ini_b_var', ini_b);
          if_el('cadForm:pos_fin_b_var', fin_b);
          if_el('cadForm:dtDados_tt:chk_sel__b_tt', 'false');
        }
        return;
    }
...
function chbSelType__a() {
    ...
    if (strbtn == 'VhblkStatusUpdate_a' ||
      ...
      if (ini_a_tp.indexOf('/') > 0) {
          ini_a_tp = ini_a_tp.substring(0,ini_a_tp.indexOf('/'));  
      }	  
      if (strbtn == 'VhblkStatusUpdate_a') {
          //alert('hello');
          if_el('cadForm:vhcle_tp_a_var', ini_a_tp);
...
function chbSelType__b() {
    //alert('chbSelType__b / strbtn:'+strbtn);
    fin_for = parseInt(fin_b) + 1;
    if (strbtn == 'VhblkStatusUpdate_b' || 
        strbtn == 'VhblkSidingCustomerDisposal_b' ||
        strbtn == 'VhblkSidingMaintenanceReceive_b') {
      ...
      if (ini_b_tp.indexOf('/') > 0) {
          ini_b_tp = ini_b_tp.substring(0,ini_b_tp.indexOf('/')); 
      }	  
      //alert('ini_b_tp: '+ini_b_tp);
      if (strbtn == 'VhblkStatusUpdate_b') {
          //alert('hello');
          if_el('cadForm:vhcle_tp_b_var', ini_b_tp);
          if_el('cadForm:vhcle_tp_b_var_desc', ini_b_tp_desc);
...
function chkBooleanslde(chk, fld) { 
  //if_el('cadForm:pjhso_cd_para1_slde', ''); 
  var k = fld.slice(fld.indexOf("para")+4, fld.indexOf("_sl")); fwdx = 1;
  //alert(k);//
  if_el('cadForm:pjhso_cd_para'+k+'_slpr', '');
  if_el('cadForm:pjhso_cd_para'+k+'_slpr_bol', 'N');
  if_el('cadForm:pjhso_cd_para'+k+'_slcc', '');
  if_el('cadForm:pjhso_cd_para'+k+'_slcc_bol', 'N');
  for (var j=1; j < 10; j++) { 
      if (k == j) { continue;} 
	  if_el('cadForm:pjhso_cd_para'+j+'_slde', '');
	  if_el('cadForm:pjhso_cd_para'+j+'_slde_bol', 'N');
  }
...
function chkBooleanslpr(chk, fld) { 
  var k = fld.slice(fld.indexOf("para")+4, fld.indexOf("_sl")); fwdx = 1;
 //alert(k);//
  if_el('cadForm:pjhso_cd_para'+k+'_slde', '');
  if_el('cadForm:pjhso_cd_para'+k+'_slde_bol', 'N');
  if_el('cadForm:pjhso_cd_para'+k+'_slcc', '');
  if_el('cadForm:pjhso_cd_para'+k+'_slcc_bol', 'N');
...
function chkBooleanslcc(chk, fld) { 
  var k = fld.slice(fld.indexOf("para")+4, fld.indexOf("_sl")); fwdx = 1;
 //alert(k);//
  if_el('cadForm:pjhso_cd_para'+k+'_slde', '');
  if_el('cadForm:pjhso_cd_para'+k+'_slde_bol', 'N');
...
function mesgStyleStub(rc,stub) {
  ...
  bck = '#99FFFF';img = 'btMessage_off.gif';
  //alert('img'+img);  
  //if (el('cadForm:iMessage')) {
  //	  alert('imgxxx'+img);  
...
function hovDtTable(dtTable) {
	/* O efeito de 'hover' foi transferido para o css */
}
...
function selDtTableRowSrc(dataTable, num) {
    setSelectedRow(num,dataTable);
}
...
/* 
 * Define o estilo da linha selecionada 
 * rowIndex: Ã¬ndice da linha selecionada (Iniciado em zero)
 * dataTable: Id do datatable 
 */
function setSelectedRow(rowIndex, dataTable) {
	if(!rowIndex || !dataTable) {
		console.log('rowIndex e dataTableId sÃ£o parÃ¢metros obrigatÃ³rios');
	}
	var trs = document.getElementById('cadForm:'+dataTable).getElementsByTagName('tbody')[0].getElementsByTagName('tr');
	
	/* Trecho que limpa a seleÃ§Ã£o anterior */
	var beforeClassName = 'tre9';
	for (i = 0; i < trs.length; i++) {
		if (trs[i].className == 'trs9') {
			trs[i].className = beforeClassName;
		}
		beforeClassName = trs[rowIndex].className == 'tre9' ? 'tro9' : 'tre9';
	}
	
	trs[rowIndex].className = 'trs9';	
}
